...
- Click Create Project.  You may get Github prompts to enter a username and password.  If so, please follow the steps in the **Authentication** section below to create a personal access token (PAT) in Github.  Once you generate your PAT, paste it into the password field (for username, use your Github username).  In the past Windows users did not see username and password prompts, but Mac users did.  After creating your first Git project in R Studio, your computer should remember your PAT, and you should not be prompted for a username and password when you create R Studio Git projects in the future.

# Authentication 

In this course, we will push and pull files from Github using HTTPS.  When you create your first git project in R Studio, you *may* be prompted for a username and password (in past classes, Mac users were, but Windows users were not).  If you are prompted for a username and password when you create your first R Studio Git project, you will need to use a personal access token (PAT) as your credential for HTTPS operations. Click on the link below for instructions on creating your PAT in Github.  You will need to be logged into your github account.  You can choose the expiration date for the PAT to be whatever you like, but I suggest making it at least 5 months from today to last the duration of this class.  For the scope of the token, you can select all options to be safe.
...
Be sure to copy this PAT to a safe location in case you need it at a later time!  
